  ...
  ngOnInit(): void {
    this.router.events.subscribe((event: any) => {
      ...
      if (event instanceof NavigationEnd) {
        this.spinner.hide()
        window.scroll(0,0);
        if (event.url === '/login' || event.url =='/register' || event.url =='/forgotPassword' || event.url == '/help' || event.url == '/help-non-member'  || this.router.url.includes('mobile-login') ||  this.router.url.includes('share')) {
          this.login= true;
